...
//Assigning action class list element to argListElement
	...
	public static synchronized void configScrCapPerAction() {
		...
		if (cmdSrcCapPerAction != null) {
			...
			srcCapPerAction = Boolean.parseBoolean(GlobalVariables.configProp.getProperty("scrCapPerAction"));
		}
	}
...
//		Thread.sleep(GlobalVariables.waitTime);
	...
	/**
	 * get value from locator excel sheet
	 * 
	 * @param key
	 * @return value
	 */
	...
	public static void sendKeys(Keys tab) {
		// TODO Auto-generated method stub
		
	}
	
	/**
     * clearing Field one by one
     *
     * @param element
     */
    public synchronized static final void clearField(WebElement element) {
        String status = "FAIL";
        try {
            new WebDriverWait(WebDriverManager.getDriver(), GlobalVariables.waitTime)
                    .until(ExpectedConditions.visibilityOf(element));
            String fieldvalue = element.getAttribute("value");
            int count = fieldvalue.length();
...
//                Thread.sleep(1000);
    ...
    public synchronized static final void enterAuth() throws InterruptedException, AWTException {
    	Thread.sleep(3000);
		Robot r= new Robot();
