...
public class Reporter {
	...
	public synchronized static void addScreenCapture(String message, String color) {
		...
		String screenshotName = CaptureScreenshot.screenCapture(message);
...
//		toDirPath =  toDirPath+"/SprintRun_"+LocalDate.now()+System.currentTimeMillis();
//		File file1 = new File(toDirPath);
//		file1.mkdir();
	...
	public static void sprintReport() {

		PropertyFileUtils property = new PropertyFileUtils("./src/test/resources/extent.properties");
		String toDirPath = "./" + property.getProperty("extent.reporter.spark.out");

		String dateTime = new DateTime().toString("dd-MM-yy HH_mm_ss");
	//	String newPath =  toDirPath+"/SprintRun_"+LocalDate.now()+System.currentTimeMillis();
		String newPath =  toDirPath+"/SprintRun_"+dateTime;

		File file1 = new File(newPath);
		file1.mkdir();
		
		//String fromDirPath = "./src/main/resources/core-images";
		File fromDir = new File(toDirPath);
		
		File[] listFiles = fromDir.listFiles();

		for (File file : listFiles) {
			try {
				if(file.getName().contains(".png") || file.getName().contains(".html"))
				{
					Files.copy(file, new File(file1 + "/" + file.getName()));
					file.delete();
				}
			} catch (IOException e) {
				e.printStackTrace();
