...
public class DesktopDriverManager {
	...
	private static synchronized void configInstalledPath() {
		/*reading from cmd line*/
		String cmdInsPath =System.getProperty("winappdriver.installation.dir");
		
		if(cmdInsPath!=null) {
			insPath = cmdInsPath;
		}else {
			/*reading from property file*/
			insPath = GlobalVariables.configProp.getProperty("winappdriver.installation.dir");
		}
	}
	...
	private static synchronized void configHost() {
		/*reading from cmd line*/
		String cmdHostName = System.getProperty("winappdriver.host");
		
		if(cmdHostName!=null) {
			host=cmdHostName;
		}else {
			/*reading from property file*/
			host=GlobalVariables.configProp.getProperty("winappdriver.host");
		}
	}
	...
	private static synchronized void configPort() {
		/*reading from cmd line*/
		String cmdPort = System.getProperty("winappdriver.port");
		if(cmdPort!=null) {
			port = cmdPort;
		}else {
			/*reading from poroperty file*/
			port = GlobalVariables.configProp.getProperty("winappdriver.port");
		}
	}
